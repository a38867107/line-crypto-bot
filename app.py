...
def get_crypto_panel(coin_name):
    ...
    if isinstance(price_res, dict) and "markPrice" in price_res:
        ...
        ticker_res = fetch_binance_futures_data(f"https://api.binance.com/api/v3/ticker/price?symbol={symbol_usdt}")
    ...
    if isinstance(oi_res, dict) and "openInterest" in oi_res:
        ...
        return f"❌ 幣安 {coin} 即時持倉量接口超時或未回應，請稍後再試。"
    ...
    def get_oi_change_str(hist_data, lookback_idx):
        if hist_data and len(hist_data) > lookback_idx:
            try:
                ...
                pass
        return "數據獲取失敗... (0.0000%)"

    # 5. 🚀 抓取幣安官方真實主動買賣大單資金流向 (5m, 1h, 4h 級別)
    taker_5m = fetch_binance_futures_data("https://fapi.binance.com/futures/data/takerlongshortRatio", {"symbol": symbol_usdt, "period": "5m", "limit": 15})
    taker_1h = fetch_binance_futures_data("https://fapi.binance.com/futures/data/takerlongshortRatio", {"symbol": symbol_usdt, "period": "1h", "limit": 30})
    taker_4h = fetch_binance_futures_data("https://fapi.binance.com/futures/data/takerlongshortRatio", {"symbol": symbol_usdt, "period": "4h", "limit": 50})

    # 計算真實主動流向（買入量減去賣出量，再換算為美元值）
    def calculate_real_inflow(taker_data, lookback_count):
        if taker_data and len(taker_data) >= lookback_count:
            try:
                net_inflow_usd = 0.0
                # 累加指定週期內的每一根 K 線的資金流入
                for i in range(1, lookback_count + 1):
                    k_data = taker_data[-i]
                    buy_vol = float(k_data.get("buyVol", 0.0))    # 主動買入量 (幣數)
                    sell_vol = float(k_data.get("sellVol", 0.0))  # 主動賣出量 (幣數)
                    
                    # 淨主動流入 (幣數) * 當時市價 = 美元流向值
                    net_inflow_usd += (buy_vol - sell_vol) * price
                
                return fmt_val(net_inflow_usd, with_sign=True, width=12)
            except:
                pass
        return "       $0.00"
    ...
    # --- 11 大時間級別幣安官方主動大單資金淨流入 ---
    net_5m   = calculate_real_inflow(taker_5m, 1)   # 1 根 5m
    net_15m  = calculate_real_inflow(taker_5m, 3)   # 3 根 5m
    net_30m  = calculate_real_inflow(taker_5m, 6)   # 6 根 5m
    net_1h   = calculate_real_inflow(taker_1h, 1)   # 1 根 1h
    net_4h   = calculate_real_inflow(taker_1h, 4)   # 4 根 1h
    net_8h   = calculate_real_inflow(taker_1h, 8)   # 8 根 1h
    net_12h  = calculate_real_inflow(taker_1h, 12)  # 12 根 1h
    net_24h  = calculate_real_inflow(taker_1h, 24)  # 24 根 1h
    net_48h  = calculate_real_inflow(taker_4h, 12)  # 12 根 4h
    net_72h  = calculate_real_inflow(taker_4h, 18)  # 18 根 4h
    net_168h = calculate_real_inflow(taker_4h, 42)  # 42 根 4h
    ...
    reply_text = (
        ...
        f"多 {acc_long:.1f}% [{gen_bar(acc_long)}] {acc_short:.1f}% 空\n\n"
        f"持仓变化 (實際價值) | 幣安總持倉：{fmt_val(binance_oi_usd, width=0)}\n"
        f"5分钟   {oi_5m}\n"
        f"15分钟  {oi_15m}\n"
        f"30分钟  {oi_30m}\n"
        f"1小時   {oi_1h}\n"
